...
func (n *mockCache) GetAll(arr *[]*cache2.KeyValue) int { return 0 }
func (n *mockCache) Get(k string) *cache2.KeyValue {
	if k == n.Key {
		return n.KV
	}
	return nil
}
func (n *mockCache) GetPrefix(prefix string, arr *[]*cache2.KeyValue) int {
	if prefix == n.Key {
		if arr != nil {
			*arr = append(*arr, n.KV)
		}
...
func (n *mockCache) ForEach(iter func(k string, v *cache2.KeyValue) (next bool)) {
	iter(n.Key, n.KV)
}
func (n *mockCache) Put(k string, v *cache2.KeyValue) {
	n.Key = k
	n.KV = v
}
...
func TestNewKvCacher(t *testing.T) {
	...
	// prepare data
	var evt cache2.KvEvent
	cr = &KvCacher{
		Cfg: cache2.Configure().
			WithEventFunc(func(e cache2.KvEvent) {
				evt = e
			}),
		ready:     make(chan struct{}),
		lw:        lw,
	...
	*cr.Cfg = old
	...
	var evts = make(map[string]cache2.KvEvent)
	lw.Rev = 0
	test.Revision = 3
	test.Kvs = nil
	...
	evt.KV = nil
	old = *cr.Cfg
	cr.Cfg.WithEventFunc(func(evt cache2.KvEvent) {
...
func BenchmarkFilter(b *testing.B) {
	...
	cfg := cache2.Configure().WithParser(proto.InstanceParser)

	n := 300 * 1000 // 30w
	cache := cache2.NewKvCache("test", cfg)
