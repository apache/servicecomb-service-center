...
func (c *InstanceCacher) getInstances(domainProject, serviceID string) (m map[string]*cache.KeyValue) {
	var arr []*cache.KeyValue
	key := core.GenerateInstanceKey(domainProject, serviceID, "")
	if l := c.Cache().GetPrefix(key, &arr); l > 0 {
		m = make(map[string]*cache.KeyValue, l)
		for _, kv := range arr {
...
func (c *InstanceCacher) onEndpointsEvent(evt K8sEvent) {
	...
	newKvs := make(map[string]*cache.KeyValue)
...
func NewInstanceCacher(c *cache.CommonCacher) (i *InstanceCacher) {
