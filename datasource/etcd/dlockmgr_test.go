...
func TestDLock(t *testing.T) {
	m1, err := datasource.Instance().NewDLock("key1", 5, true)
	...
	ch := make(chan bool)
	go func() {
		m2, err := datasource.Instance().NewDLock("key1", 1, false)
		assert.Nil(t, m2)
		assert.Error(t, err)
		fmt.Println("m2 try failed")
		m2, err = datasource.Instance().NewDLock("key1", 1, true)
		assert.Nil(t, m2)
		assert.Error(t, err)
	...
	<-ch
	m3, err := datasource.Instance().NewDLock("key1", 2, true)
	assert.NoError(t, err)
	assert.NotNil(t, m3)
