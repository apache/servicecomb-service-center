...
func (ds *MetadataManager) RegisterService(ctx context.Context, request *pb.CreateServiceRequest) (
	...
	if err != nil {
		log.Error(fmt.Sprintf("create micro-service[%s] failed, init schemas failed, operator: %s",
			serviceFlag, remoteIP), err)
		return nil, err
	}
	...
	defer func() {
		if err != nil {
			log.Error(fmt.Sprintf("create micro-service[%s] failed, init schemas failed, start roolback ...",
				serviceFlag, remoteIP), err)
...
func (ds *MetadataManager) UnregisterService(ctx context.Context, request *pb.DeleteServiceRequest) (err error) {
	...
	defer func() {
		if err != nil {
			log.Error(fmt.Sprintf("%s micro-service[%s] failed, clean local schmea dir failed, operator: %s, start rollback ...",
				title, serviceID, remoteIP), err)
