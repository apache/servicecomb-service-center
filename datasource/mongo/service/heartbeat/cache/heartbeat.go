...
func cleanInstance(ctx context.Context, serviceID string, instanceID string) error {
	...
	filter := mutil.NewFilter(mutil.InstanceServiceID(serviceID), mutil.InstanceInstanceID(instanceID))
	ins, err := getInstance(ctx, filter)
	if err != nil {
