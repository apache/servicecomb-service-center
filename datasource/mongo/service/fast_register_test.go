...
import (
	...
	"github.com/apache/servicecomb-service-center/datasource/mongo/service"
)

func TestInstance_BatchCreate(t *testing.T) {
	var serviceID string

	service.FastRegConfiguration().QueueSize = 100
	fastRegisterService := service.NewFastRegisterInstanceService()
	service.SetFastRegisterInstanceService(fastRegisterService)

	fastRegisterTimeTask := service.NewRegisterTimeTask()
	...
	t.Run("when instance request is 3 expect batch register success", func(t *testing.T) {
		...
		for i := 0; i < instanceBatchLen; i++ {
			event := &event.InstanceRegisterEvent{Ctx: context.TODO(), Request: request}
			fastRegisterService.AddEvent(event)
		}
		assert.Equal(t, instanceBatchLen, len(service.GetFastRegisterInstanceService().InstEventCh))
		time.Sleep(500 * time.Millisecond)

		assert.Equal(t, 0, len(service.GetFastRegisterInstanceService().InstEventCh))
		assert.Equal(t, 0, len(service.GetFastRegisterInstanceService().FailedInstCh))
	...
	t.Run("when instanceID is custom expect register success", func(t *testing.T) {
		...
		assert.Equal(t, 1, len(service.GetFastRegisterInstanceService().InstEventCh))
		time.Sleep(500 * time.Millisecond)
