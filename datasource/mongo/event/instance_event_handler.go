...
func NotifySyncerInstanceEvent(evt sd.MongoEvent, ms *discovery.MicroService) {
	...
	log.Info(fmt.Sprintf("instanceId in NotifySyncerInstanceEvent : %v, the serviceId : %v", instance.InstanceId, instance.ServiceId))
	instanceKey := util.StringJoin([]string{datasource.InstanceKeyPrefix, evt.Value.(sd.Instance).Domain,
		evt.Value.(sd.Instance).Project, instance.ServiceId, instance.InstanceId}, datasource.SPLIT)
	...
	serviceKey := util.StringJoin([]string{datasource.ServiceKeyPrefix, evt.Value.(sd.Instance).Domain,
		evt.Value.(sd.Instance).Project, instance.ServiceId}, datasource.SPLIT)
	serviceKv := dump.KV{
		Key:   serviceKey,
		Value: ms,
	}
	...
	log.Debug(fmt.Sprintf("success to add instance change event with adtion [%s], instanceKey : %s to event queue", instEvent.Action, instanceKey))
}
