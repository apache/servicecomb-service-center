...
type ConfigDistributor interface {
	Create(ctx context.Context, policy *model.Policy) ([]byte, error)
	Update(ctx context.Context, p *model.Policy) error
	Delete(ctx context.Context) error
	Display(ctx context.Context) ([]byte, error)
...
func Create(ctx context.Context, spec *model.Policy) ([]byte, error) {
	for _, cd := range distributors {
		return cd.Create(ctx, spec)
	}
	return nil, nil
}

func List(ctx context.Context) ([]byte, error) {
	for _, cd := range distributors {
		return cd.List(ctx)
	}
	return nil, nil
...
func Display(ctx context.Context) ([]byte, error) {
	for _, cd := range distributors {
		return cd.Display(ctx)
	}
	return nil, nil
}
...
func Delete(ctx context.Context) error {
	for _, cd := range distributors {
		return cd.Delete(ctx)
	}
	return nil
}

func Update(ctx context.Context, p *model.Policy) error {
	for _, cd := range distributors {
		return cd.Update(ctx, p)
	}
	return nil
}
