...
func Init(opts Options) error {
	...
	if !ok {
		return fmt.Errorf("dependency datasource implementation not supported [%s]", opts.PluginImplName)
	}
	var err error
	...
	log.Info(fmt.Sprintf("dependency datasource mgt [%s@%s] enabled", opts.PluginImplName, opts.Endpoint))
	return nil
}
