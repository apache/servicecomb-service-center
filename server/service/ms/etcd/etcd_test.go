...
func TestService_Register(t *testing.T) {
	t.Run("register service with nil data", func(t *testing.T) {
	...
	t.Run("register service with too long parameter, should failed", func(t *testing.T) {
		ms.Install("etcd", func(opts ms.Options) (ms.DataSource, error) {
			return etcd.NewDataSource(), nil
		})
	...
	t.Run("same serviceId,different service, can not register again,error is same as the service register twice",
		func(t *testing.T) {
			...
			assert.NotNil(t, resp)
			assert.NoError(t, err)
			assert.Equal(t, resp.Response.GetCode(), scerr.ErrServiceAlreadyExists)
		})
}
...
func TestSchema_Create(t *testing.T) {
	...
	t.Run("create schemas out of gauge", func(t *testing.T) {
		...
		log.Info("should fail when create service")
		respCreateService, err := ms.MicroService().RegisterService(getContext(), &pb.CreateServiceRequest{
			Service: &pb.MicroService{
				AppId:       "check_schema_group_service_ms",
				ServiceName: "check_schema_service_service_ms",
				Version:     "1.0.0",
				Level:       "FRONT",
				Schemas:     schemaIds,
				Status:      pb.MS_UP,
		...
		assert.NoError(t, err)
		assert.NotEqual(t, proto.Response_SUCCESS, respCreateService.Response.GetCode())
	})
