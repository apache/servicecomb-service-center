...
func (n *mockCache) GetPrefix(prefix string, arr *[]*discovery.KeyValue) int {
	if prefix == n.Key {
		if arr != nil {
			*arr = append(*arr, n.KV)
		}
...
func (n *mockCache) Put(k string, v *discovery.KeyValue) {
	n.Key = k
	n.KV = v
}
...
func TestNewKvCacher(t *testing.T) {
	...
	*cr.Cfg = old
	...
	var evts = make(map[string]discovery.KvEvent)
	lw.Rev = 0
	test.Revision = 3
	test.Kvs = nil
